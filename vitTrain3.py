...
def plot_training_results(train_losses, test_losses, train_psnrs, test_psnrs):
    ...
    save_path = 'maeVitResultsTemporalSpatialSmoothing/Train-TestGraph2.png'
...
def save_checkpoint(model, optimizer, epoch, train_losses, test_losses, filename="maeVitCheckpointTimeSolar+sptailsmoothing.pth"):
...
def load_checkpoint(model, optimizer, filename="maeVitCheckpointTimeSolar+sptailsmoothing.pth"):
...
def train_model(model, train_dataloader, test_dataloader, num_epochs, learning_rate, mask_threshold = 0.9, checkpoint_path="maeVitCheckpointTimeSolar+sptailsmoothing.pth"):
    model.to(device)
