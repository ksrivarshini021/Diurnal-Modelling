...
from GRUBaselineArch import GRUImagePredictor
...
def masked_mae_loss(pred, target, mask_value=-1, spatial_lamda=0.001, temporal_lamda=0.001):
    ...
    loss = torch.sum(torch.abs(pred - target)) / (mask.sum() + 1e-8)  
    #for smoothing
    ...
    if pred.size(0) > 1:
        ...
        temporal_diff = torch.tensor(0.0, dtype=pred.dtype, device=pred.device)
...
def plot_lst_heatmaps(lst_data, is_target=False, epoch=None):
    ...
    if is_target:
        plt.savefig(f'GRUBaseline/target_epoch{epoch+1}.png')
    else:
        plt.savefig(f'GRUBaseline/predicted_epoch{epoch+1}.png')
    plt.close()
...
def save_checkpoint(model, optimizer, epoch, train_losses, test_losses, filename="GRUBaseline.pth"):
...
def load_checkpoint(model, optimizer, filename="GRUBaseline.pth"):
...
def train_model(model, train_dataloader, test_dataloader, num_epochs, learning_rate, mask_threshold = 0.9, checkpoint_path="GRUBaseline.pth"):
    model.to(device)
