...
def plot_lst_heatmaps(lst_data, is_target=False, epoch=None):
    ...
    if is_target:
        plt.savefig(f'BiLSTMbaseline/target_epoch{epoch+1}.png')
    else:
        plt.savefig(f'BiLSTMbaseline/predicted_epoch{epoch+1}.png')
    plt.close()
...
def save_checkpoint(model, optimizer, epoch, train_losses, test_losses, filename="BiLSTMBaseline2.pth"):
...
def load_checkpoint(model, optimizer, filename="BiLSTMBaseline2.pth"):
...
def train_model(model, train_dataloader, test_dataloader, num_epochs, learning_rate, mask_threshold = 0.9, checkpoint_path="BiLSTMBaseline2.pth"):
    model.to(device)
