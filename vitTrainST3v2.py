...
from preTrainedImproved3 import maeViT
...
class SatelliteImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return input_image, target_tensor, temporal_info, spatial_info     
...
def plot_training_results(train_losses, test_losses, train_psnrs, test_psnrs):
    ...
    save_path = 'TestResults/Train-TestGraph2.png'
...
def plot_lst_heatmaps(lst_data, is_target=False, epoch=None):
    ...
    if is_target:
        plt.savefig(f'TestResults/target_epoch{epoch+1}.png')
    else:
        plt.savefig(f'TestResults/predicted_epoch{epoch+1}.png')
    plt.close()
...
def test_model(model, test_dataloader, epoch=None):
    ...
    # printed_plot = False  
    with torch.no_grad():
        for batch_idx, (inputs, targets, temporal_info, sptaial_info) in enumerate(test_dataloader):
            inputs, targets = inputs.to(device), targets.to(device)
            temporal_info, sptaial_info = temporal_info.to(device), sptaial_info.to(device)
            outputs = model(inputs, temporal_info, sptaial_info)
...
def save_checkpoint(model, optimizer, epoch, train_losses, test_losses, filename="maeVitCheckpointTest.pth"):
...
def load_checkpoint(model, optimizer, filename="maeVitCheckpointTest.pth"):
...
def train_model(model, train_dataloader, test_dataloader, num_epochs, learning_rate, mask_threshold = 0.9, checkpoint_path="maeVitCheckpointTest.pth"):
    model.to(device)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, (inputs, targets, temporal_info, spatial_info) in enumerate(train_dataloader):
